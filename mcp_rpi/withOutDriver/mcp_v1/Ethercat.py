...
class EtherCATInterface:
    ...
    def read_fifo(self):
        ...
        self.write_reg(ECAT_PRAM_RD_ADDR_LEN,
                       0x00201000)  # we always read 32 bytes (0x0020), output process ram offset 0x1000
        self.write_reg(ECAT_PRAM_RD_CMD, 0x80000000)  # start command

        TempLong = ULONG()
        TempLong.LANLong = self.read_reg(ECAT_PRAM_RD_CMD, 4)

        while (not (TempLong.LANByte[0] & PRAM_READ_AVAIL) or (
                TempLong.LANByte[1] != 8)):  # Corrected logical OR operator
            TempLong.LANLong = self.read_reg(ECAT_PRAM_RD_CMD, 4)
    ...
    def write_fifo(self):
        ...
        self.write_reg(ECAT_PRAM_WR_ADDR_LEN,
                       0x00201200)  # we always write 32 bytes (0x0020), input process ram offset 0x1200
